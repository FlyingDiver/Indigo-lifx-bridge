...
def parse_packet(data):
    ...
    logger.debug(u"parse_packet()")


    # Frame Header
    frame_header_size = section_size(frame_header) / 8
    logger.debug(u"parse_packet(), frame_header_size = %i", frame_header_size)

    frame_header_data = data[0:frame_header_size]
    logger.debug(u"parse_packet(), frame_header_data = %s", repr(frame_header_data))

    frame_header_struct = unpack_section(
            frame_header,
            frame_header_data
    )
    logger.debug(u"parse_packet(), frame_header_struct.size = %i", frame_header_struct.size)
